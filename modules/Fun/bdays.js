	...
	execute: async function (client, message, args) {
		if (!args[1]) { // bdays, bdays user
			if (!args[0] || args[0] == 'list' || args.join(' ') == 'sort date') { // bdays
				...
				message.channel.send(`${user.name}'s birthday is in <t:${Math.floor(user.date / 1000)}>'`);
			}
		} else {
			switch (args[0]) {
				case 'add': {
					...
					if (!toadd) return message.channel.send('That\'s not a valid name homie.');
					const date = new Date(args.slice(1).join(' '));
					if (isNaN(date)) return message.channel.send('Wha');
					await client.birthdays.create({
						name: toadd,
						date
					});
					message.channel.send(`Added ${toadd}'s motherfucking birthday!`);
					break;
				}
				case 'remove':
				case 'rm':
				...
				case 'delete': {
					const user = await client.birthdays.findOne({ where: { name: { [like]: args[1] } } });
					if (!user) return message.channel.send('This user doesn\'t even exist...');
					await user.destroy();
					message.channel.send(`Deleted ${args[1]}'s birthday entry. Poor g.`);
					break;
				}
				case 'sort': {
					switch (args[1]) {
						case 'name': {
							const dbData = (await client.birthdays.findAll());
							if (dbData.length === 0) return message.channel.send('There is *nothing*, add something to be the first, gangsta!');
							const longestName = getLongestString(dbData.map(function (e) { return e.name })).length;
